...
//! OS-restart of the background service.
...
impl<R: Runtime> BackgroundService<R> for CfmsBackgroundService {
    ...
    ///
    /// The heavy lifting, such as token refresh, download queue, and lockdown
    /// monitor, is owned by the [`ServiceManager`] started in `lib.rs`. This
    /// method ensures the platform does not suspend those tasks by holding the
    ...
    async fn run(&mut self, ctx: &ServiceContext<R>) -> Result<(), ServiceError> {
        ...
        // --- Ensure ServiceManager is activated ---
        //
        ...
        {
            let state = app_handle.state::<AppHandleState>();
            let service_manager = Arc::clone(&state.service_manager);
